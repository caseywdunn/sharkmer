...
- Active panel references now require exact extraction receipts against `panels/reference_sources.json.gz`; all 159 old entries are retained separately as regression-only history. The [initial provenance audit](../dev_docs/REFERENCE_PROVENANCE.md) found 100 source-exact candidates and 59 source mismatches, then excluded two known gene-annotation conflicts. The [annotation-reviewed replacement follow-up](../dev_docs/PUBLIC_REFERENCE_REPLACEMENTS.md) expands 99 to 156 active public regions, including partial and related-taxon evidence; three quarantined entries remain unresolved. Source mismatches alone do not establish biological incorrectness.
